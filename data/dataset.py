...
class COCODataset(torch.utils.data.Dataset):
    ...
    def pull_item(self, index):
        ...
        #load image for preprocessing if in memory, otherwise load from directory and add to memory
        if self.imgs is not None:
            img = self.imgs[index]
        else:
            dirname = getDataDir()
            dataset_name = self.data_dir.split('/')
            img_file = os.path.join(dirname, dataset_name[len(dataset_name)-1] +'/'+ self.name+'/'+ file_name)
            img = cv2.imread(img_file)

            assert img is not None, "error img {}".format(file_name)
