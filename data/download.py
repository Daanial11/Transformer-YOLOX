...
parser.add_argument('--check_valid', required=False, default=False, help="check file integrity, if bad redownload")
    ...
    for i in range(nr_images):
        ...
        if not os.path.isfile(file_path):
            ...
            if img._getexif():
                ...
                img.save(file_path)"""
            os.system("wget -q " + url_original + " -O " + file_path)
...
if(args.check_valid):
    ...
    for i in range(nr_images):
        ...
        try:
            ...
            print(file_name + "invalid, redownloading")
            response = requests.get(url_original)
            img = Image.open(BytesIO(response.content))
            if img._getexif():
                img.save(file_path, exif=img.info["exif"])
    ...
    sys.stdout.write('Integrity Check Finished\n')
